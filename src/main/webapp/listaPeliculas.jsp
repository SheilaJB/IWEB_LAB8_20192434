...
<form action="listaPeliculas" method="POST">
    <div class="combobox-container">
        ...
        <label>Seleciona tus opciones: </label>
        <select name=id_genero"  >
            <% for(genero movie : listaGeneros) {%>
            <option values="<%=movie.getIdGenero()%>">
                <%=movie.getNombre()%>
            </option>
            <% }%>
        </select>
        <select name=id_Streaming"  >
            <% for(streaming movie : listaStreaming) {%>
            <option values="<%=movie.getIdStreaming()%>">
                <%=movie.getNombreServicio()%>
            </option>
            <% }%>
        </select>
        <input type="hidden" name="action" value="filtrar">
        <button type="submit" href="<%=request.getContextPath()%>/PeliculaServlet?action=filtrar">Filtrar</button>
        <form action="listaPeliculas?action=listar" method="GET">
            <button type="submit">Limpiar</button>
        </form>
        <button class="btn btn-success" href="<%=request.getContextPath()%>/PeliculaServlet?action=new">Crear Pelicula</button>
    </div>
